...
int findmax(int l, int c)
{
    return l>c?l:c;
}

int findmin(int l, int c)
{
    return l<c?l:c;
}
...
int main()
{
    ...
    while(completed!=r)
    {
        //finding the  process with min. burst time in ready queue at current time
        int min_index = -1;
        int minimum = INT_MAX;
        for(int i = 0; i < r; i++) {
    ...
    //Calculateing the  Length of Process completion cycle
